...
DEPLOYMENT_LOG="$DEPLOY_LOG_DIR/deployment_$TIMESTAMP.log"

# Initialize logging - capture all output to deployment log
    ...
    if [[ $rc -ne 0 ]]; then
        {
            ...
            echo ""
            echo "--- docker volumes ---"
            $SUDO docker volume ls 2>&1 || true
            echo ""
            echo "--- obsidian logs (last 100 lines) ---"
            $SUDO $COMPOSE_CMD logs --tail=100 obsidian 2>&1 || true
            echo ""
            echo "--- oauth2-proxy logs (last 100 lines) ---"
            $SUDO $COMPOSE_CMD logs --tail=100 oauth2-proxy 2>&1 || true
            echo ""
...
stop_containers() {
    ...
    if $SUDO docker network ls | grep -q "obsidian-net"; then
        ...
        $SUDO docker network rm obsidian-net 2>&1 | while read -r line; do log_debug "$line"; done || true
    fi

    # Remove obsidian-specific volumes (named volumes from compose)
    log_info "Removing obsidian volumes ..."
    $SUDO docker volume ls -q | grep -E "obsidian" | xargs -r $SUDO docker volume rm 2>&1 | while read -r line; do log_debug "$line"; done || true
...
deploy() {
    ...
    log_info "Step 3: Removing network..."
    if $SUDO docker network ls | grep -q "obsidian-net"; then
        $SUDO docker network rm obsidian-net 2>&1 | while read -r line; do log_debug "$line"; done || true
    fi
    ...
    log_info "Step 4: Waiting for cleanup to complete..."
    sleep 5
